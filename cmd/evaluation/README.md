...
- 层得分 = 检查项加权平均；报告按三个决策问题给出**三条结论**（`sections`），替代单一加权总分作为判定依据：
    - **接入与合规**（L1/L2/L6）：全部 Passed → `pass`；任一 fail → `fail`（接入问题无灰色地带，直接阻断）
    - **性能画像**（L5）：Passed → `pass`；未达标 → `warn`（SLO 未达只是提醒接入方选型，不阻断）
    - **可用性冒烟**（L3/L4）：全部 Passed → `pass`；任一 fail → `warn`（冒烟失败表示能力/稳定性不足，不是接入阻断项）
- 主结论（`verdict`）由三条结论推导，退出码由**接入结论**驱动：
    - 接入 `pass` 且冒烟 `pass`/`na` → `pass`，退出码 0
    - 接入 `pass` 且冒烟 `warn` → `pass_with_warnings`，退出码 0
    - 接入 `fail` → `fail`，退出码 1
    - L1 门控失败 → `abort`，退出码 1
    - 未执行任何层 → `no_layers_executed`
- `total_score` 保留为全层加权参考分（L1 10%，L2 15%，L3 30%，L4/L5/L6 各 15%），**仅供展示，不参与判定**
- 报告写入 `reports/<时间戳>/report.json`（含 `sections`/`summary` 与全部原始指标，供 CI 消费）与 `report.md`，终端同步打印汇总
- `report.md` 结构：一句话摘要 → 体检结论表 → 关键信号 → 逐层明细 → 原始指标附录 → 裁判总结（可选，见下）

### 裁判总结

配置 `judge` 段后，报告末尾会附加一段不超过 300 字的中文总结，由裁判模型生成（复用 judge 配置，无需额外配置）。未配置 judge 时跳过；生成失败只标注错误、不影响退出码。
...
- **streaming_sse**：首内容 chunk（含思考内容）延迟占总耗时超过 90%（且总耗时 >2s）时，detail 会提示疑似伪流式转发（流式对降低体感延迟无效），不影响得分
