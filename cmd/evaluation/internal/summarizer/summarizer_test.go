...
type fakeProvider struct {
	resp *provider.Result
	err  error
}

func (f *fakeProvider) Chat(_ context.Context, _ *provider.Request) (*provider.Result, error) {
	return f.resp, f.err
}
func (f *fakeProvider) Stream(_ context.Context, _ *provider.Request) (*provider.Result, error) {
	return f.resp, f.err
}
...
func TestSummarizeNormal(t *testing.T) {
	s := New(&fakeProvider{resp: &provider.Result{Content: "该模型可以接入，性能需关注。"}})
...
func TestSummarizeStripFence(t *testing.T) {
	s := New(&fakeProvider{resp: &provider.Result{Content: "```text\n该模型可用。\n```"}})
	text, err := s.Summarize(context.Background(), buildReport())
	if err != nil {
		t.Fatalf("Summarize: %v", err)
	}
...
func TestSummarizeEmpty(t *testing.T) {
	s := New(&fakeProvider{resp: &provider.Result{Content: ""}})
...
func TestSummarizeTruncate(t *testing.T) {
	...
	s := New(&fakeProvider{resp: &provider.Result{Content: long}})
	text, err := s.Summarize(context.Background(), buildReport())
	if err != nil {
		t.Fatalf("Summarize: %v", err)
	}
