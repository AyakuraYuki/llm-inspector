...
- **版本一致性**：协议当前为 **v3**（v2 起 `think_time`/`max_output_tokens` 随任务下发；v3 新增请求数制分片、本地分词器、数据集、无界开环、每档预热）。coordinator 与所有 agent 必须来自 **同一次构建**——版本不一致会在探活阶段直接中止，而不是让旧 agent
...
第二、三批对标 evalscope 的能力在分布式下的落点：

| 配置项                                  | 分布式语义                                                                                                                                                                        |
|-----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `tokenizer.path`                        | coordinator 下发的是**路径**而非词表本体（词表数 MB，不走 HTTP）。**每台 agent 必须预置同一份词表目录**；coordinator 随配置下发词表文件的 SHA-256，agent 在预检阶段校验本地指纹，不一致或目录缺失即中止 |
| `prompt.mode: dataset`                  | coordinator 读文件后把全部行**内联**在任务配置里下发，agent 无需预置文件、也不存在版本不一致；数据集很大（数十 MB）时每档下发都会携带，请自行评估                                 |
| `prompt.tokens_min/max`                 | 透传，各 agent 各自逐请求采样                                                                                                                                                     |
| `requests_per_level`                    | 全局请求数按并发份额非零的 agent 等分（`SplitAmongActive`），没拿到 worker 的 agent 不分配额，保证全局恰好发出配置的数量；报表回填全局上限                                        |
| `open_loop_unbounded` / `warmup_per_level` | 透传；每档预热由 coordinator 按本档并发/速率切分下发 warmup 任务                                                                                                                  |
| Steady / Last 30s / usage 对拍          | 纯 metrics 层，在 coordinator 合并样本后计算，与单机版口径逐字段一致                                                                                                              |

## 运行流程

1. **探活**：逐台 `ping`，校验协议版本一致且空闲，任一不可达即中止
2. **会话建立**：下发各节点在整个 run 中的最大并发分片，agent 据此一次性配置连接池
3. **预检**： **每台 agent** 都对全部模型做一次连通性预检（验证各机到上游的网络路径），任一失败即中止；终端里模型名会标注 `@agent地址`。配置了 `tokenizer` 时，agent 先校验本地词表可加载且指纹与 coordinator 一致，失败以 `(agent) 地址` 形式报出
